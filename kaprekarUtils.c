...
int isKaprekar(int n) {
  ...
  if(n < 1) {
    return 0; //Function expects a returned int, 'false' is not an int
  }
  ...
  long modulus = 1; //Should this be 1? //ERROR PROPOSAL NOT FINAL
  long first, second;

  //for each possible "split" of the square...
  for(i=1; i<=numDigits; i++) { //variable declared as 'numDigits', not 'numberOfDigits'
    //increase the modulus by a factor of 10
    modulus *= 10;  //variable defined as 'modulus', not 'modulous'
    ...
    if(second > 0 && n == first + second) {  //logical comparisons require '==', not '='
      return 1;
    }
  }
  return 0;
